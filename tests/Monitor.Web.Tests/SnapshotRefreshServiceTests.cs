...
public sealed class SnapshotRefreshServiceTests
{
    ...
    [Fact]
    public async Task RetainedStaleFallback_IsNotReportedAsSuccessfulRefresh()
    {
        var repository = new InMemoryServerRegistrationRepository();
        var registration = Registration();
        repository.Upsert(registration);
        var cache = new FakeCache(registration.Id, SnapshotFreshness.Stale);
        var service = new SnapshotRefreshService(repository, cache, new FixedTimeProvider());

        var result = await service.RefreshAsync(registration.Id);

        Assert.Equal(SnapshotRefreshStatus.RetainedStale, result.Status);
        Assert.Equal(SnapshotFreshness.Stale, result.Freshness);
        Assert.Contains("retained stale", result.Message, StringComparison.OrdinalIgnoreCase);
        Assert.Equal(1, cache.RefreshCount);
    }
    ...
    private sealed class FakeCache(Guid id, SnapshotFreshness freshness = SnapshotFreshness.Fresh) : IServerHealthSnapshotCache
    {
        ...
        private readonly SnapshotCacheResult _result = new(
            ...
            freshness,
            freshness == SnapshotFreshness.Fresh ? TimeSpan.Zero : TimeSpan.FromMinutes(1));
