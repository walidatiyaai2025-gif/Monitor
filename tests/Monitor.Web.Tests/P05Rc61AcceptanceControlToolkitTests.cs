...
public sealed class P05Rc61AcceptanceControlToolkitTests
{
    ...
    [Fact]
    public void Toolkit_DefinesExactlySixAcceptanceControlScripts()
    {
        var text = Read("deploy/RC61_ACCEPTANCE_CONTROL_TOOLKIT.md");
        foreach (var file in RequiredFiles)
            Assert.Contains($"`scripts/{file}`", text, StringComparison.Ordinal);
        Assert.Contains("Do not add a seventh acceptance-control script", text, StringComparison.Ordinal);
    }

    [Fact]
    public void Toolkit_RequiresExactImmutableReviewedCommitRatherThanMovingRef()
    {
        var text = Read("deploy/RC61_ACCEPTANCE_CONTROL_TOOLKIT.md");
        Assert.Contains("exact final PR #259 head", text, StringComparison.OrdinalIgnoreCase);
    ...
    [Fact]
    public void Toolkit_BindsCommitAndSixFileHashesIntoLockedSession()
    {
    ...
    [Fact]
    public void ProductionRunbook_SeparatesSidecarAcceptanceControlsFromCandidateBundledDeploymentTools()
    {
