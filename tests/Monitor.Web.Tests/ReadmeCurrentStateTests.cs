...
public sealed class ReadmeCurrentStateTests
{
    ...
    public void Readme_ReflectsCurrentBatchAndP05State()
    {
        ...
        Assert.Contains("BATCH-100 through BATCH-800 are complete", readme, StringComparison.Ordinal);
        Assert.Contains("BATCH-800/#287 is closed completed", readme, StringComparison.Ordinal);
        ...
        Assert.Contains("docs/IMPLEMENTATION_PLAN.md` is retained historical implementation context", readme, StringComparison.Ordinal);
        Assert.Contains("must not be used to reopen already-merged work", readme, StringComparison.Ordinal);
