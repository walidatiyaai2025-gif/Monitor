...
public sealed class EnterpriseReportsController : Controller
{
    ...
    [HttpGet("/reports/database-health/{registrationId:guid}.csv")]
    public async Task<IActionResult> DatabaseHealth(Guid registrationId, CancellationToken cancellationToken)
    {
        if (registrationId == Guid.Empty) return NotFound();
        var model = await _monitoring.GetServerAsync(registrationId.ToString("D"), cancellationToken);
        if (model is null) return NotFound();
    ...
    [HttpGet("/reports/backup-health.csv")]
    public IActionResult BackupHealth()
    {
    ...
    [HttpGet("/reports/performance-health.csv")]
    public IActionResult PerformanceHealth()
    {
        return Download(_reports.PerformanceHealth(), "text/csv; charset=utf-8", EnterpriseDownloadSubject.PerformanceHealth, "csv");
    }
