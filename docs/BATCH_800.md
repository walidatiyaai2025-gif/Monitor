...
| Reports | versioned CSV/ZIP/JSON endpoints | **Existing exports remain wired; B800-081 adds a Viewer+ bounded/versioned/redacted Fleet decision-support CSV, B800-082 adds a Viewer+ per-server/per-operation Maintenance decision-support CSV that keeps target identity request-only and unavailable evidence explicit, B800-083 adds a contextual Viewer+ cached Server Intelligence CSV, B800-084 adds a contextual Viewer+ cached Database Health summary CSV, B800-085 completes their selected-Server-Details download workflow, B800-086 adds a contextual Viewer+ cached Memory Health summary CSV, and B800-087 adds an estate-wide Viewer+ cached Backup Health aggregate CSV with RPO compliance deliberately `NotEvaluated`** |
...
- B800-087 stale concurrent-work head `bcd2426eb214f91c382807061e7b8493b4776486` in PR #320 passed CI #2714 / `32074763524` and Windows production-candidate #492 / `32074763486` on its old base; those runs are provenance only. The unique Backup Health changes were clean-ported onto post-B800-086 main in PR #322, and #320 was closed unmerged. Clean implementation head `e355965fc949b04e50c1cc1bb85476a2719974fa` passed CI #2753 / `32078032443` and Windows production-candidate #503 / `32078032437`; Real SQL was not selected because no monitored-SQL query, collector or permission path changed. Exact final post-canonical-reconciliation validation remains required.

## Current B800-087 merge gate

PR #322 is the current focused B800 slice and closes B800-087 only. `docs/FEATURE_CATALOG.md`, `docs/STATUS.md`, `docs/IMPLEMENTATION_PLAN.md`, and this ledger must describe B800-086 / PR #321 as merged and B800-087 as the bounded cached Backup Health estate export slice. Before PR #322 is marked Ready or merged, every repository-selected required workflow must be Green on the same exact final reconciled head, the branch must remain current with `main`, review threads must remain resolved, and the effective diff must stay bounded to B800-087 implementation/tests/work-note plus canonical reconciliation. Real SQL is required only if repository path policy selects it. Issue #287 remains OPEN for B800-088+. Stale PR #320 is closed unmerged and superseded by the clean current-main port in PR #322.
