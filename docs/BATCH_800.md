...
| Reports | versioned CSV/ZIP/JSON endpoints | **Existing exports remain wired; B800-081 adds a Viewer+ bounded/versioned/redacted Fleet decision-support CSV, and B800-082 adds a Viewer+ per-server/per-operation Maintenance decision-support CSV that keeps target identity request-only and unavailable evidence explicit** |
...
- [x] B800-082 add a Viewer+ bounded/versioned/redacted per-server/per-operation Maintenance decision-support CSV that shares the visible page evidence builder, keeps target identity request-only, exports unavailable/NotEvaluated truth explicitly and adds no maintenance execution or monitored-SQL path (`docs/work/B800-082.md`, PR #314).
- [ ] B800-083..090 continue bounded, versioned, redacted exports only where existing evidence and operator value justify them.
...
## Current B800-082 merge gate

PR #314 is the current focused B800 slice and closes B800-082 only. `docs/FEATURE_CATALOG.md`, `docs/STATUS.md`, `docs/IMPLEMENTATION_PLAN.md`, and this ledger must describe B800-081 / PR #313 as merged and B800-082 as the current bounded Maintenance decision-support export slice. Before PR #314 is marked Ready or merged, every repository-selected required workflow must be Green on the same exact final reconciled head, the branch must remain current with `main`, review threads must remain resolved, and the effective diff must stay bounded to B800-082 implementation/tests/work-note plus canonical reconciliation. Real SQL is required only if repository path policy selects it. Issue #287 remains OPEN for B800-083+.