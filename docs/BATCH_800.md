...
| Reports | versioned CSV/ZIP/JSON endpoints | **Existing exports remain wired; B800-081 adds a Viewer+ bounded/versioned/redacted Fleet decision-support CSV, B800-082 adds a Viewer+ per-server/per-operation Maintenance decision-support CSV that keeps target identity request-only and unavailable evidence explicit, B800-083 adds a contextual Viewer+ cached Server Intelligence CSV, B800-084 adds a contextual Viewer+ cached Database Health summary CSV, B800-085 completes their selected-Server-Details download workflow, and B800-086 adds a contextual Viewer+ cached Memory Health summary CSV through the existing shared report contract** |
...
- B800-086 implementation head `7a5209401c001a4d2c68907195139dac8635598a` passed CI #2735 / `32076637425` and Windows production-candidate #498 / `32076638430` end-to-end; Real SQL was not selected because this slice changes no monitored-SQL query, collector or permission path. Final merge evidence must come from the later canonical-reconciled exact head.

## Current B800-086 merge gate

PR #321 is the current focused B800 slice and closes B800-086 only. `docs/FEATURE_CATALOG.md`, `docs/STATUS.md`, `docs/IMPLEMENTATION_PLAN.md`, and this ledger must describe B800-085 / PR #319 as merged and B800-086 as the contextual cached Memory Health export slice. Before PR #321 is marked Ready or merged, every repository-selected required workflow must be Green on the same exact final reconciled head, the branch must remain current with `main`, review threads must remain resolved, and the effective diff must stay bounded to B800-086 implementation/tests/work-note plus canonical reconciliation. Real SQL is required only if repository path policy selects it. Issue #287 remains OPEN for B800-087+. PR #320 is preserved as deferred B800-087 Backup Health work but must not merge from its stale branch without a current-main port and fresh exact-head gates.
