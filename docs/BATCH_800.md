...
| Reports | versioned CSV/ZIP/JSON endpoints | **Existing exports remain wired; B800-081 adds a Viewer+ bounded/versioned/redacted Fleet decision-support CSV, B800-082 adds a Viewer+ per-server/per-operation Maintenance decision-support CSV that keeps target identity request-only and unavailable evidence explicit, B800-083 adds a contextual Viewer+ cached Server Intelligence CSV that reuses the Server Details projection and remains browser-cache-only, and B800-084 adds a direct Viewer+ bounded cached Database Health estate CSV that reuses the B300 database-state projection while keeping aggregate and retained-detail availability independent** |
...
B800-083 extends the report contract to contextual cached Server Intelligence without creating a second truth model. Viewer+ `GET /reports/server-intelligence/{registrationId:guid}.csv` resolves the selected registration through `IMonitorReadService.GetServerAsync`, which uses registration/control-plane state plus snapshot-cache reads; the browser GET does not invoke a monitored-SQL query or collector. The export reuses `ServerIntelligenceProjection.Build(model)` from Server Details and the existing `monitor-export-v2` row/byte/cell/formula-safety contract. Missing snapshot, database, runtime-pressure or other snapshot-derived facts remain explicit `Unavailable` instead of placeholder zero/healthy; a complete cached snapshot may truthfully serialize observed numeric zeroes. Credentials, connection strings, SQL text/plans, client/table data, raw provider errors and physical filesystem paths are excluded. Reports & Diagnostics exposes the export contextually through server selection. Exact final head `067de7549b7758bc680ccfb595ed66848d69f637` passed CI `32072383956` / #2653, Real SQL `32072384083` / #343 and Windows production-candidate `32072384098` / #479; PR #315 squash-merged as `301c6af20534d37a899d8f8e3d50c81d7494ebb4`.

B800-084 adds a direct estate-wide cached Database Health report without widening collection or creating a second classification model. Viewer+ `GET /reports/database-health.csv` reads only `IMonitorReadService.GetHealthModulesAsync`, reuses `DatabaseStateProjection.Build(...)`, and serializes at most 250 deterministic server rows plus one explicit coverage/truncation row through `monitor-export-v2`. Aggregate online/total and aggregate state counters remain independent from retained per-database row availability: valid aggregate evidence stays numeric while retained-state classification fields remain `Unavailable` when exact retained rows are absent. Individual database names, registration IDs, credentials, connection strings, SQL text/plans, provider payloads, table data and physical filesystem paths are excluded. No monitored-SQL query/collector/permission, refresh, mutation or remediation path is added. Implementation head `bfdd35b78051f00a85a39cb695651b0d3f987324` passed CI `32073239580` / #2685 and Windows production-candidate `32073239609` / #485; Real SQL was not selected because this slice changes no monitored-SQL query, collector or permission path.
...
- [x] B800-084 add a direct Viewer+ bounded/versioned/redacted cached Database Health estate CSV that reuses `DatabaseStateProjection`, caps output at 250 server rows plus explicit coverage/truncation metadata, keeps aggregate evidence independent from retained-row availability and excludes database names/registration IDs (`docs/work/B800-084.md`, PR #317).
- [ ] B800-085..090 continue bounded, versioned, redacted exports only where existing evidence and operator value justify them.
...
Bounded cached Database Health estate export slice:
- `src/Monitor.Web/Services/DatabaseHealthExport.cs`
- `src/Monitor.Web/Controllers/EnterpriseReportsController.cs`
- `src/Monitor.Web/Services/EnterpriseSecurityPolicy.cs`
- `src/Monitor.Web/Views/Portal/Reports.cshtml`
- `tests/Monitor.Web.Tests/B800DatabaseHealthExportTests.cs`
- `tests/Monitor.Web.Tests/B800DatabaseHealthExportEvidenceBoundaryTests.cs`
...
- B800-083 implementation/discoverability head `300b2e99590d742a3936efbf209febd5e79bad4f` passed CI #2637; exact final reconciled head `067de7549b7758bc680ccfb595ed66848d69f637` passed CI `32072383956` / #2653, Real SQL `32072384083` / #343 and Windows production-candidate `32072384098` / #479; PR #315 squash-merged as `301c6af20534d37a899d8f8e3d50c81d7494ebb4`.
- B800-084 exact implementation head `bfdd35b78051f00a85a39cb695651b0d3f987324` passed CI `32073239580` / #2685 and Windows production-candidate `32073239609` / #485 end-to-end. Real SQL was not selected because this slice changes no monitored-SQL query, collector or permission path. Final merge evidence must come from the later canonical-reconciled exact head.

## Current B800-084 merge gate

PR #317 is the current focused B800 slice and closes B800-084 only. `docs/FEATURE_CATALOG.md`, `docs/STATUS.md`, `docs/IMPLEMENTATION_PLAN.md`, and this ledger must describe B800-083 / PR #315 as merged and B800-084 as the direct bounded cached Database Health estate export slice. Before PR #317 is marked Ready or merged, every repository-selected required workflow must be Green on the same exact final reconciled head, the branch must remain current with `main`, review threads must remain resolved, and the effective diff must stay bounded to B800-084 implementation/tests/work-note plus canonical reconciliation. Real SQL is required only if repository path policy selects it. Issue #287 remains OPEN for B800-085+.
