...
B800-083 is complete and merged through PR #315 as `301c6af20534d37a899d8f8e3d50c81d7494ebb4`. Viewer+ contextual `GET /reports/server-intelligence/{registrationId:guid}.csv` resolves the selected registration through `IMonitorReadService.GetServerAsync`, so browser GET remains registration/control-plane + snapshot-cache only, and the export calls the same `ServerIntelligenceProjection.Build(model)` used by Server Details. Missing snapshot-derived database/runtime-pressure/health facts remain `Unavailable`; sensitive monitored-SQL and filesystem payloads remain excluded. Exact final reconciled head `067de7549b7758bc680ccfb595ed66848d69f637` passed CI #2653 / `32072383956`, Real SQL #343 / `32072384083`, and Windows production-candidate #479 / `32072384098` before squash merge.

PR #316 carries B800-084. Viewer+ contextual `GET /reports/database-health/{registrationId:guid}.csv` resolves the selected registration through the same cache-only `IMonitorReadService.GetServerAsync` path and uses `DatabaseStateProjection.Build(...)` only for retained-state summary values. Aggregate online/total and database-state counters stay independent from retained-row availability; missing retained rows emit `Unavailable` rather than reconstructed states. Retained database names and registration IDs are not serialized. The shared `monitor-export-v2` bounds/formula-safety and secure download policy remain authoritative; no collector, refresh, mutation, remediation, failover or configuration write is introduced.

B800-084 implementation head `1895f4c340ee835a2a9e1aef3f401905ae136def` passed CI #2674 / `32073015882` and Windows production-candidate #480 / `32073015903`; Real SQL was not selected because no monitored-SQL query, collector or permission path changed. **Ready/merge still requires all four canonical files reconciled and every repository-selected required workflow Green on the same exact final post-reconciliation head**, no unresolved review threads, branch current with `main`, and a diff bounded to B800-084 implementation/tests/work-note/canonical reconciliation. #287 remains OPEN for B800-085+.
...
- BATCH-800: IN PROGRESS under #287; incremental focused slices are merged through B800-083, with current PR #316 carrying B800-084. Excluded from completed-task totals until its own batch gates close.
...
- B800-084 exports only a contextual cached database-health summary through the shared bounded CSV contract; unavailable snapshot or retained-state evidence remains explicit, retained database names/registration IDs are excluded, aggregate and retained evidence remain separate, and the GET adds no monitored-SQL, refresh, mutation or remediation path.
...
**Overall:** 🟢 verified foundation · 🟢 P0.1–P0.4 COMPLETE · 🟢 P0.5 repository cutover/evidence/session/finalization/release/promotion/workflow-supply-chain/native-Node-24/durable-release hardening through PR #219 · 🟢 #256 selected-product-hash session hardening COMPLETE via PR #257 · 🟢 #258/#259 locked-session sidecar binding COMPLETE · 🟢 #261/#262 Acceptance Control Toolkit provenance COMPLETE with exact toolkit source `b422eaaee53d931a62a43b3c36a53b68cd4f3e27` · 🟢 #266/#267 RC.61 read-only promotion preflight COMPLETE · 🟢 #270/#271 Step 0 operator handoff COMPLETE · 🟢 #276/PR #279 IIS bootstrap integration COMPLETE · 🟢 #281/PR #283 fresh-host/PowerShell 7 hardening COMPLETE · 🟢 #285/PR #286 clean IIS no-demo/POST-error fix COMPLETE · 🟡 #287 BATCH-800 IN PROGRESS: focused slices merged through B800-083; PR #316 carries B800-084 bounded cached Database Health summary export and awaits exact final-head validation after canonical reconciliation · 🟡 selected RC.61 durable publication + separate verification pending manual #162 · ⛔ #116 production mutation blocked while #162 is OPEN · 🟡 external IIS/HTTPS 15-gate acceptance pending after #162 · 🔴 production acceptance not yet granted