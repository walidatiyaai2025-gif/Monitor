...
## Incident-note file-state cross-process safety — #449 / PR #450 CLOSURE CANDIDATE

**Base:** `main@c0d6472b522548a8cf4ad4d2d6271ad722dd0f86`.  
**Gap:** SingleNode `FileIncidentNoteRequestStateStore` previously cached shard state per object/process and used only an instance-local lock. `AtomicJsonFile.Save` kept each JSON file physically atomic but did not serialize the complete read -> mutate -> write transaction across independent workers sharing the same operational root. A stale worker could therefore double-claim an already armed request, lose peer shard entries or overwrite durable `Applied` with stale `Armed`.  
**Closure candidate:** PR #450 removes file-shard caching, uses one bounded `incident-note-requests.lock` `FileShare.None` mutation lease across the authoritative disk reload/validation/mutation/atomic persist sequence, fails closed after bounded lease contention, and carries independent-store regressions for peer Armed claims, lost entries, Applied monotonicity and lease blocking. SharedState CAS semantics and the existing 64-shard / 512-entry bounds remain unchanged.  
**Validation state:** exact docs-inclusive PR #450 head must be current with `main`, have zero unresolved review threads, and pass Linux CI, Windows production-candidate, SQL Server 2022 Real SQL when selected/required, plus both protected-P0 guards before merge.  
...
**Tracking reconciliation:** PR #450 replaces the stale post-merge `#447/#448 CLOSURE CANDIDATE` wording that remained immediately after PR #448 merged.
