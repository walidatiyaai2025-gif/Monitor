...
- Issue #276 / PR #278 **IN REVIEW**: packaged Windows Server/IIS bootstrap installer adds PLAN ONLY default inspection, online/offline PowerShell 7 + IIS feature + .NET 8 Hosting Bundle setup, exact approved-certificate/PFX validation, low-privilege No Managed Code app-pool/site/binding/ACL bootstrap, and a one-command installer that delegates to the existing preflight/versioned deployment/rollback tooling. Repository-only; no #162 dispatch and no #116 production mutation/acceptance.
