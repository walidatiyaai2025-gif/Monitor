...
**Live-state authority:** exact GitHub `main` and current PR/issue/workflow evidence first, then `AGENTS.md` and `docs/CURRENT_EXECUTION_PLAN.md`. Historical implementation detail must not reopen merged work. The full status ledger that preceded this live convergence is preserved byte-for-byte at `docs/history/STATUS_PRE_WM7_RECONCILIATION_2026-09-09.md`.

## Current repository state — 2026-09-09

Fresh convergence base before the current optional-browser reconciliation:
...
- Website Monitoring #463/#466: **COMPLETE / MERGED** through PR #467 -> PR #464. The required runtime is closed and is not reopened by optional browser verification.

## Optional Website Monitoring browser verification — PR #472

PR #472 / `task/website-monitoring-wm7-browser-evidence` is an **optional verification-only** line. It adds a real authenticated Chromium path without changing Website Monitoring runtime authority, monitored SQL behavior, production deployment or external acceptance.

Recovery work on the existing branch repaired three concrete historical failures rather than creating duplicate implementation:

1. `dotnet run` previously honored the Development launch profile and ignored the workflow-owned loopback port; the harness now uses `--no-launch-profile`.
2. the browser dependency install previously used unnecessary OS-package mutation; the isolated harness now pins Playwright `1.63.0`, requires `npm audit --audit-level=high`, and installs Chromium without `--with-deps`.
3. the first real browser execution exposed an ambiguous `Name` selector; the script now uses exact accessible roles and labels while retaining Playwright strictness.

Before the current canonical tracking reconciliation, exact head `fce84a2c6743061d6ff402d3e6ba1d9e66ac1db6` was current with `main` (`behind_by=0`), had zero unresolved review threads, and passed:
...
The real browser run retained artifact `10117386057` with digest `sha256:6fa8cdd0bb834d4907c0bfd8fe73c32d1068f118476ebc06bd0f8ad8b6dac697`.

That evidence is recovery provenance only. This tracking reconciliation changes the head, so final integration still requires fresh exact-head Green CI, fresh exact-head Green browser evidence, both protected-P0 guards, `behind_by=0`, and zero unresolved review threads. Live GitHub state controls whether PR #472 is currently open, Ready or merged.
...
- repository CI, browser evidence, release tooling and documentation cannot manufacture OWNER_ONLY or EXTERNAL PASS.

## Completion status

**Repository/runtime implementation:** complete except for any legitimate live integration work currently visible in GitHub.  
