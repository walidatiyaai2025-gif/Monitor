...
PR #315 carries B800-083. Viewer+ contextual `GET /reports/server-intelligence/{registrationId:guid}.csv` resolves the selected registration through `IMonitorReadService.GetServerAsync`, so browser GET remains registration/control-plane + snapshot-cache only, and the export calls the same `ServerIntelligenceProjection.Build(model)` used by Server Details. `EnterpriseReportContract` retains `monitor-export-v2`, 1000-row/1-MiB/500-char-cell limits and formula-safe CSV. Missing snapshot-derived database/runtime-pressure/health facts are serialized as `Unavailable` rather than placeholder zero/healthy; a complete cached snapshot may truthfully emit observed numeric zeroes. Credentials, connection strings, SQL text/plans, client/table data, raw provider errors and physical paths remain excluded. Reports & Diagnostics advertises the contextual export and routes the operator through server selection.

B800-083 implementation/discoverability head `300b2e99590d742a3936efbf209febd5e79bad4f` passed CI #2637; Windows production-candidate #471 was still running before canonical reconciliation. **Ready/merge requires all four canonical files reconciled and every repository-selected required workflow Green on the same exact final post-reconciliation head**, no unresolved review threads, branch current with `main`, and a diff bounded to B800-083 implementation/tests/work-note/canonical reconciliation. Real SQL is required only when repository path policy selects it. #287 remains OPEN for B800-084+.
...
- BATCH-800: IN PROGRESS under #287; incremental focused slices are merged through B800-082, with current PR #315 carrying B800-083. Excluded from completed-task totals until its own batch gates close.
...
**Overall:** 🟢 verified foundation · 🟢 P0.1–P0.4 COMPLETE · 🟢 P0.5 repository cutover/evidence/session/finalization/release/promotion/workflow-supply-chain/native-Node-24/durable-release hardening through PR #219 · 🟢 #256 selected-product-hash session hardening COMPLETE via PR #257 · 🟢 #258/#259 locked-session sidecar binding COMPLETE · 🟢 #261/#262 Acceptance Control Toolkit provenance COMPLETE with exact toolkit source `b422eaaee53d931a62a43b3c36a53b68cd4f3e27` · 🟢 #266/#267 RC.61 read-only promotion preflight COMPLETE · 🟢 #270/#271 Step 0 operator handoff COMPLETE · 🟢 #276/PR #279 IIS bootstrap integration COMPLETE · 🟢 #281/PR #283 fresh-host/PowerShell 7 hardening COMPLETE · 🟢 #285/PR #286 clean IIS no-demo/POST-error fix COMPLETE · 🟡 #287 BATCH-800 IN PROGRESS: focused slices merged through B800-082; PR #315 carries B800-083 bounded cached Server Intelligence export and awaits exact final-head validation after canonical reconciliation · 🟡 selected RC.61 durable publication + separate verification pending manual #162 · ⛔ #116 production mutation blocked while #162 is OPEN · 🟡 external IIS/HTTPS 15-gate acceptance pending after #162 · 🔴 production acceptance not yet granted