...
## Incident-note durable idempotency + replay authority — #445/#446 COMPLETE · #447/#448 CLOSURE CANDIDATE

**Canonical reconciliation:** PR #446 / Issue #445 merged durable bounded `Armed` / `Applied` incident-note request state into `main` but its canonical `STATUS.md`, `IMPLEMENTATION_PLAN.md` and `FEATURE_CATALOG.md` delta was omitted. PR #448 reconciles that historical omission together with the follow-up replay-truth gap in #447.  
**#445 / PR #446:** COMPLETE / MERGED as `b4365058bd9809f080070f2d440f358083938262`; incident-note request identity now survives rolling-audit eviction/restart through bounded durable SingleNode/SharedState stores, legacy retained receipts are materialized on startup, `Applied` wins over `Armed`, and saturation remains fail-closed. Exact PR #446 head `367d1b929b64abcba28596289ac7a16cf2be72a1` passed normal CI, SQL Server 2022 Real SQL, Windows production-candidate and both protected-P0 guards before merge.  
**#447 / PR #448:** implementation + deterministic regression are present on `agent/447-durable-note-replay-authority`. Coordinated incident-note request state is now authoritative over legacy rolling-audit preflight, so a request durably advanced to `Applied` before a failing final audit append replays as an idempotent no-op rather than false ambiguity; durable `Armed` stays fail-closed and plain non-coordinated `IAuditStore` behavior stays unchanged.  
**Validation state:** exact docs-inclusive head must be current with `main`, have zero unresolved review threads, and pass Linux CI, Windows production-candidate and both protected-P0 guards before PR #448 can leave Draft/merge. SQL Server 2022 Real SQL must be Green when selected/required by repository policy; this closure changes no monitored-target SQL query/permission path.  
**Boundary:** no monitored-target SQL query/permission expansion, credential behavior, autonomous remediation, RC.61 publication, production IIS/SQL mutation, external P0 acceptance or branch-protection mutation. Manual/external order remains `#162 -> #116 -> #111`; #353 remains repository-admin only.
