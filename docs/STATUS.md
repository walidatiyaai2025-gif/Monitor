...
## Issue #276 / PR #279 — IIS bootstrap installer follow-up — IN PROGRESS

- `Bootstrap-IisProductionSingleNode.ps1` and `Install-ProductionSingleNode.ps1` are now present on `main` from concurrent repository work, with static fail-closed coverage in `P05IisBootstrapInstallerTests.cs`.
- The bootstrap remains **PLAN ONLY by default** and requires explicit elevated `-Apply` for approved server changes. It supports Offline and constrained Microsoft Online .NET 8 Hosting Bundle paths, optional SHA-256 pinning, Authenticode verification, existing machine-thumbprint or SecureString PFX certificate flows, low-privilege No Managed Code app-pool/site/binding setup, stable roots and ACLs.
- The installer preserves `bootstrap -> Test-IisProductionPrerequisites.ps1 -> Deploy-ProductionSingleNode.ps1`; the authoritative preflight and existing immutable-release/external-App_Data/rollback semantics are not replaced.
- Main CI run `31998946661` exposed exactly two repository integration gaps after the new tests landed: production-candidate parser/package integration and `docs/DEPLOY_IIS.md` operator coverage. PR #279 addresses both and adds the canonical PLAN/STATUS/CATALOG tracking required by project policy.
- PR #279 must pass normal CI and Windows production-candidate before merge. This work does not publish or alter RC.61, does not touch real IIS/SQL, and does not satisfy #162/#116/#111.

**Overall:** 🟢 verified foundation · 🟢 P0.1–P0.4 COMPLETE · 🟢 P0.5 repository cutover/evidence/session/finalization/release/promotion/workflow-supply-chain/native-Node-24/durable-release hardening through PR #219 · 🟢 #256 selected-product-hash session hardening COMPLETE via PR #257 · 🟢 #258/#259 locked-session sidecar binding COMPLETE · 🟢 #261/#262 Acceptance Control Toolkit provenance COMPLETE with exact toolkit source `b422eaaee53d931a62a43b3c36a53b68cd4f3e27` · 🟢 #266/#267 RC.61 read-only promotion preflight COMPLETE · 🟢 #270/#271 Step 0 operator handoff COMPLETE · 🟡 #276/PR #279 IIS bootstrap packaging/docs/tracking follow-up in validation · 🟡 selected RC.61 durable publication + separate verification pending manual #162 · ⛔ #116 production mutation blocked while #162 is OPEN · 🟡 external IIS/HTTPS 15-gate acceptance pending after #162 · 🔴 production acceptance not yet granted
