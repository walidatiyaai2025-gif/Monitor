...
- Issue #276 / PR #278 — Windows Server/IIS bootstrap installer **IN REVIEW**: plan-first `Setup-MonitorServer.ps1` plus one-command `Install-Monitor.ps1` detect/install PowerShell 7, IIS scripting features and the .NET 8 Hosting Bundle; support offline prerequisite paths; require the exact approved HTTPS certificate; create/validate the low-privilege No Managed Code app pool/site/binding/ACL baseline; package the setup tooling under `_operations`; then delegate to the existing authoritative preflight/deploy/rollback path. Repository-only preparation; it does not dispatch #162 or mutate/accept #116 production.
...
5. Run the packaged `Install-Monitor.ps1` in PLAN ONLY mode to inspect prerequisite/IIS state; after approval use explicit `-Apply` to bootstrap missing prerequisites and delegate to the existing authoritative IIS preflight/versioned deployment/rollback tooling.
