...
- B800-087 exposes Viewer+ estate-wide `GET /reports/backup-health.csv` from enabled registrations plus `IServerHealthSnapshotCache.Peek(...)` only and uses the existing `EnterpriseReportContract` / `monitor-export-v2` owner;
- B800-087 serializes cached freshness/collection time and existing full-backup covered/missing/latest aggregate evidence only; missing cache/backup evidence and cache-read failure remain explicit `Unavailable`, while legitimate observed zeroes remain numeric zeroes;
- B800-087 keeps `ComplianceState=NotEvaluated` whenever aggregate backup evidence exists because the snapshot lacks the per-database recovery-model/full/log evidence required for B300 compliance, excludes database names and registration IDs, and adds no monitored-SQL/collector/permission/refresh/backup/restore/mutation/remediation/failover/configuration-write path;
- B800-087 clean-port implementation head `e355965fc949b04e50c1cc1bb85476a2719974fa` passed CI #2753 / `32078032443` and Windows production-candidate #503 / `32078032437`; Real SQL was not selected because no monitored-SQL query, collector or permission path changed. Stale PR #320 is closed unmerged after its unique work was preserved in current PR #322. Exact final post-canonical-reconciliation validation remains required.
...
Least privilege remains read-only: SQL Server 2022+ uses `VIEW SERVER PERFORMANCE STATE` (older supported versions `VIEW SERVER STATE`) plus `VIEW ANY DEFINITION` and existing narrow metadata grants; Agent history/activity adds only read-only `SELECT` on `msdb.dbo.sysjobhistory` and `msdb.dbo.sysjobactivity`, with no SQLAgent execution/operator role. B800-071/072/073/074/075/076/077/078/079/080/081/082/083/084/085/086/087 add no monitored-SQL permission or query path.

PR #322 becomes eligible for Ready/merge only after `BATCH_800`, `FEATURE_CATALOG`, `STATUS` and this plan are reconciled on one exact head, every repository-selected required workflow is Green on that same head, review threads are resolved, the branch is current with `main`, and the effective diff remains bounded to B800-087 plus canonical reconciliation. Real SQL is required only if repository path policy selects it. Merging #322 closes B800-087 only; #287 remains OPEN for B800-088+. PR #320 is closed unmerged and superseded by the clean current-main port in #322.
...
- `docs/BATCH_800.md` — B800-001..100 IN PROGRESS under #287; incremental focused slices are merged through B800-086 on `main`, with PR #322 carrying B800-087. The overall batch is not counted as complete.
