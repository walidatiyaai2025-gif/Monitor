...
Incremental focused slices have advanced the batch beyond the historical #288 partial branch. Current `main` contains the evidence-backed server/diagnostic/workflow slices plus B800-071 fleet decision support, B800-072 maintenance safety decision support, B800-073 bounded incident decision evidence, B800-074 repository-bounded incident operator reads, B800-075 persisted/decorated native incident reads, B800-076 Fleet operator-policy availability, B800-077 Maintenance operator-policy availability, B800-078 bounded Fleet incident risk, B800-079 full bounded Fleet routing coverage, B800-080 full bounded Fleet correlation coverage and B800-081 bounded Fleet decision-support export; PR #314 carries B800-082 bounded Maintenance decision-support export.
...
- B800-082 exposes Viewer+ `GET /reports/maintenance-decision-support/{registrationId:guid}.csv?operation=...` through the same `EnterpriseReportContract`, keeps target identity in the request route only and shares `MaintenanceDecisionSupport.BuildEvidence(...)` with the visible page;
- B800-082 preserves unreadable policy, truncated incident population, approval, rollback, approved-window, replica-health and recent-backup facts as nullable `Unavailable`/`NotEvaluated` evidence; only existing deterministic B400 decision fields are serialized when evaluation is possible;
- B800-082 deliberately excludes registration/server identity, display name, group/tags, maintenance-window reason, incident IDs/rules, assignees, credentials, connection strings, SQL text/plans, raw provider errors, monitored-SQL payloads and filesystem paths;
- B800-082 initial implementation head `54ad82e6cca6a7afe0e7438af591f20a59b2c3c8` built successfully but CI `32067364921` / #2616 exposed one stale B800-077 source-contract assertion after evidence ownership moved into the shared helper; the test was corrected at its new shared owner without weakening behavior or safety semantics;
- B800-082 corrected implementation head `be35bf0d5e06fefa8706edb55b6cc9879c2b6533` passed CI `32067526388` / #2618 and Windows production-candidate `32067525698` / #464 end-to-end; Real SQL was not selected because no monitored-SQL query/collector/permission path changed.
...
Least privilege remains read-only: SQL Server 2022+ uses `VIEW SERVER PERFORMANCE STATE` (older supported versions `VIEW SERVER STATE`) plus `VIEW ANY DEFINITION` and existing narrow metadata grants; Agent history/activity adds only read-only `SELECT` on `msdb.dbo.sysjobhistory` and `msdb.dbo.sysjobactivity`, with no SQLAgent execution/operator role. B800-071/072/073/074/075/076/077/078/079/080/081/082 add no monitored-SQL permission or query path.

PR #314 becomes eligible for Ready/merge only after `BATCH_800`, `FEATURE_CATALOG`, `STATUS` and this plan are reconciled on one exact head, every repository-selected required workflow is Green on that same head, review threads are resolved, the branch is current with `main`, and the effective diff remains bounded to B800-082 plus canonical reconciliation. Real SQL is required only if repository path policy selects it. Merging #314 closes B800-082 only; #287 remains OPEN for B800-083+.
...
- `docs/BATCH_800.md` — B800-001..100 IN PROGRESS under #287; incremental focused slices are merged through B800-081 on `main`, with PR #314 carrying B800-082. The overall batch is not counted as complete.
