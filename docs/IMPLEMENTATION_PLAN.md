...
- PR #267 / Issue #266 — read-only RC.61 durable-promotion operator preflight **IN VERIFICATION**: exact selected repository/version/source run/artifact/outer digest/product hash/source head/tested merge/tag are pinned; source-run/artifact provenance and expiry fail closed; exact approved promotion and separate verifier commands are emitted. The helper never dispatches, creates/mutates a release or tag, rebuilds/repackages RC.61, touches IIS/SQL or satisfies #162/#116/#111.
...
2. Run the read-only #266 preflight from a trusted authenticated operator checkout, then complete the manual exact-existing-candidate durable publication workflow on #162 and independently verify tag/assets/hash without rebuilding RC.61.
