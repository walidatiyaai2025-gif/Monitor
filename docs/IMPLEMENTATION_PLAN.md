...
- B800-083 is complete and merged through PR #315 as `301c6af20534d37a899d8f8e3d50c81d7494ebb4`; exact final reconciled head `067de7549b7758bc680ccfb595ed66848d69f637` passed CI `32072383956` / #2653, Real SQL `32072384083` / #343 and Windows production-candidate `32072384098` / #479;
- B800-083 exposes Viewer+ contextual `GET /reports/server-intelligence/{registrationId:guid}.csv` through `IMonitorReadService.GetServerAsync` and the same `ServerIntelligenceProjection.Build(model)` used by Server Details; it retains `monitor-export-v2` bounds/formula safety, preserves unavailable snapshot/database/runtime-pressure truth instead of synthetic zero/healthy, excludes credentials/connection strings/SQL text/plans/provider payloads/physical paths, and adds no refresh/mutation/remediation or monitored-SQL browser path;
- B800-084 exposes Viewer+ direct `GET /reports/database-health.csv` from `IMonitorReadService.GetHealthModulesAsync` cache/control-plane evidence only and reuses the same `DatabaseStateProjection.Build(...)` classification owner used by Database Health;
- B800-084 caps output at 250 deterministic server rows plus one explicit coverage/truncation row, keeps aggregate online/total/state-count evidence independent from retained per-database classification availability, excludes individual logical database names/registration IDs/sensitive payloads, and adds no refresh/mutation/remediation or monitored-SQL browser path;
- B800-084 exact implementation head `bfdd35b78051f00a85a39cb695651b0d3f987324` passed CI `32073239580` / #2685 and Windows production-candidate `32073239609` / #485; Real SQL was not selected because no monitored-SQL query, collector or permission path changed. Exact final post-canonical-reconciliation validation remains required.
...
PR #317 becomes eligible for Ready/merge only after `BATCH_800`, `FEATURE_CATALOG`, `STATUS` and this plan are reconciled on one exact head, every repository-selected required workflow is Green on that same head, review threads are resolved, the branch is current with `main`, and the effective diff remains bounded to B800-084 plus canonical reconciliation. Real SQL is required only if repository path policy selects it. Merging #317 closes B800-084 only; #287 remains OPEN for B800-085+.
...
- `docs/BATCH_800.md` — B800-001..100 IN PROGRESS under #287; incremental focused slices are merged through B800-083 on `main`, with PR #317 carrying B800-084. The overall batch is not counted as complete.
