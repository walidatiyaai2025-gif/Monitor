...
- B800-086 exposes Viewer+ contextual `GET /reports/memory-health/{registrationId:guid}.csv` through cache-only `IMonitorReadService.GetServerAsync(...)` and the existing `MemoryIntelligenceProjection.Build(...)` owner; it serializes only existing SQL/OS memory, configuration/counter and dominant-clerk evidence through `monitor-export-v2`;
- B800-086 keeps missing snapshots and optional counters explicit `Unavailable`, retains valid observed zeroes, exposes contextual `Memory CSV` only for non-empty GUID-backed registrations, and adds no collector/monitored-SQL query/permission, refresh, tuning, mutation, remediation, failover or configuration-write path;
- B800-086 exact implementation head `7a5209401c001a4d2c68907195139dac8635598a` passed CI #2735 / `32076637425` and Windows production-candidate #498 / `32076638430`; Real SQL was not selected because no monitored-SQL query, collector or permission path changed. Exact final post-canonical-reconciliation validation remains required.
...
Least privilege remains read-only: SQL Server 2022+ uses `VIEW SERVER PERFORMANCE STATE` (older supported versions `VIEW SERVER STATE`) plus `VIEW ANY DEFINITION` and existing narrow metadata grants; Agent history/activity adds only read-only `SELECT` on `msdb.dbo.sysjobhistory` and `msdb.dbo.sysjobactivity`, with no SQLAgent execution/operator role. B800-071/072/073/074/075/076/077/078/079/080/081/082/083/084/085/086 add no monitored-SQL permission or query path.

PR #321 becomes eligible for Ready/merge only after `BATCH_800`, `FEATURE_CATALOG`, `STATUS` and this plan are reconciled on one exact head, every repository-selected required workflow is Green on that same head, review threads are resolved, the branch is current with `main`, and the effective diff remains bounded to B800-086 plus canonical reconciliation. Real SQL is required only if repository path policy selects it. Merging #321 closes B800-086 only; #287 remains OPEN for B800-087+. PR #320 is preserved as deferred B800-087 Backup Health work but must be ported onto current main with fresh exact-head gates before merge.
...
- `docs/BATCH_800.md` — B800-001..100 IN PROGRESS under #287; incremental focused slices are merged through B800-085 on `main`, with PR #321 carrying B800-086. The overall batch is not counted as complete.
