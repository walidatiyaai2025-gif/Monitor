...
- Issue #176 is **IN VERIFICATION**: repository toolchain reproducibility is being hardened by locking .NET SDK `8.0.424` in root `global.json` with `rollForward=disable`, making core Ubuntu gates install that repository lock explicitly, and replacing mutable `mcr.microsoft.com/mssql/server:2022-latest` with exact digest `sha256:ba4c8329f48fb8f02e1416be6a930ebfd71268caee78aa985f3af4315e457c89` captured from Green Real SQL run `31882003433`. Regression coverage rejects SDK drift and mutable SQL image refs. RC.61 and external/manual acceptance semantics are unchanged.
...
7. Issue #176 reproducible SDK/SQL-image hardening is in verification and remains repository-side only;
8. RC.61 remains selected unless #116 explicitly selects another equivalently verified candidate;
9. real Windows/IIS 15/15 acceptance remains pending external.
