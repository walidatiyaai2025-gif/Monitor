...
- Issue #180 is **IN VERIFICATION**: repository NuGet restore provenance is being made self-contained with root `nuget.config`, `<packageSources><clear />`, exactly `https://api.nuget.org/v3/index.json` using protocol v3, and wildcard package source mapping `* -> nuget.org`. Regression coverage parses the config and rejects ambient/additional sources or mapping drift. No package versions or product behavior are changed.
...
9. Issue #180 repository NuGet source provenance hardening is in verification and remains repository-side only;
10. RC.61 remains selected unless #116 explicitly selects another equivalently verified candidate;
11. real Windows/IIS 15/15 acceptance remains pending external.
