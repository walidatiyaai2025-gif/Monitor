...
- Issue #182 is **IN VERIFICATION**: direct PackageReference declarations are being placed under an explicit fail-closed allowlist covering all five direct dependencies across the two solution projects. The guard requires exact package IDs and exact semantic versions and rejects added/removed/unreviewed dependencies, wildcards, ranges, MSBuild-variable versions or silent direct-version drift. No current package version is changed.
...
10. Issue #182 direct PackageReference allowlist hardening is in verification and remains repository-side only;
11. RC.61 remains selected unless #116 explicitly selects another equivalently verified candidate;
12. real Windows/IIS 15/15 acceptance remains pending external.
