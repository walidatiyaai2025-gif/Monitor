...
## Website Monitoring convergence — #463 / PR #464 + WM-6 #466 / PR #467

**Authority and merge order:** recover the existing `feature/website-monitoring-foundation` / PR #464 and stacked `task/website-monitoring-wm6` / PR #467 only. PR #468 is validation-only. First converge and merge #467 into #464; only then validate the combined #464 head against `main`. No parallel Website Monitoring implementation is permitted.  
**WM-1..WM-5 implementation contract:** bounded target model and CRUD; explicit default-disabled monitoring; DNS/TCP/TLS/HTTP/status/redirect/content/latency/certificate evidence; fail-closed outbound policy including DNS re-resolution and per-hop authorization; durable SingleNode target/history/schedule/check state; confirmation/recovery/reopen incident lifecycle with Unknown never silently resolving an active incident; bounded linked dependency correlation that never upgrades correlation into proven root cause; recipient groups; SMTP credentials resolved only from configured environment-secret references; durable outbox with event deduplication, bounded retry and dead-letter; `/websites` management plus cached live evidence UI; attributable audit + antiforgery + named role boundaries.  
**WM-6 implementation contract:** when shared operational state is selected, targets/history/schedule/check state/groups/outbox use bounded SharedState documents and compare/exchange mutation ownership. Scheduled and manual checks converge on one execution service that owns probe -> history -> incident reconciliation -> notification planning, with same-process single-flight plus distributed per-target leases when coordination is enabled. MultiNode activation fails closed without both shared operational state and distributed coordination; platform-wide DeploymentReadiness is not bypassed.  
**Regression recovery:** the B800 manual-write acceptance follows `IWebsiteProbeExecutionService.TryExecuteAsync` while still requiring actor validation and `website.probe.manual.requested` audit evidence before outbound work. Registration tests enforce current WM-6 reality instead of stale “blocked until WM-6” assumptions.  
**Visual acceptance:** there is no browser/Playwright screenshot harness in this repository. Close the repository visual gap through dedicated route/source regression for truthful UI states, RBAC/antiforgery, cached-only live behavior, reduced motion, responsive 1180/760/520/390 contracts and no browser probe initiation. Do not claim screenshots or interactive browser validation that did not occur.  
**Exact-head gate:** pre-canonical head `dde69979bd2e5a274431a1097cafb0f569eb4c55` passed CI `34342666795`, Real SQL `34342666781`, Windows production-candidate `34342666818`, protected-P0 commits `34342666917`, and protected-P0 metadata `34342666769`; those runs are implementation evidence only after later visual/docs commits. The final WM-6 head must pass the same selected gates and have zero unresolved review threads before #467 becomes Ready or merges. The resulting combined #464 head must then pass its own exact final gates and review check before merge to `main`.  
**Default-disabled/P0 boundary:** `WebsiteMonitoring:Enabled=false`, `WebsiteNotifications:Enabled=false`, and empty private allowlist remain shipped defaults. Do not publish RC.61, mutate production IIS/SQL, manufacture SMTP-provider acceptance or infer any external P0 gate from repository CI. The independent production order remains `#162 -> #116 -> #111`.
