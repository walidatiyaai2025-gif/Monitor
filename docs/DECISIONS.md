...
## ADR-040 — Reliability verification uses deterministic fault injection over runtime contracts
B100-071..080 verifies HA behavior without making CI depend on a live SQL Server, network timing or machine-specific sleeps. Tests provide a thread-safe in-memory implementation of the same `ISharedStateDocumentStore` compare/exchange contract, a wrapper that injects explicit unavailable writes, a toggleable `ISharedStateSqlBackend`, and a mutable `TimeProvider`. Runtime repositories, lease management, readiness and refresh services are exercised unchanged against those boundaries.

The acceptance contract is state/invariant based: failed writes are atomic, stale lease owners lose authority after expiry, provider outages degrade readiness without secret disclosure, interrupted import can restart safely, optimistic concurrent transitions do not manufacture invalid state, parallel audit/history/registration writes preserve bounded semantics, and distributed refresh admits at most one collector. The soak test drives multiple logical nodes through repeated registration/audit/history/incident/lease cycles while asserting bounded state and deduplication rather than wall-clock performance.

Passing this simulation does not itself enable production MultiNode. The deployment-readiness gate remains authoritative and all secret/key/cache/security prerequisites must still be configured and healthy before MultiNode startup is allowed.

## ADR-041 — Production deployment is versioned, least-privilege and health-gated
B100-081..090 treats deployment material as executable product surface rather than informal notes. Production configuration templates contain no credential values, connection strings, Admin hashes/salts or key material. IIS and Windows Service deployments use dedicated low-privilege identities; Monitor opts into the official .NET Windows Service lifetime so the published executable can run under Service Control Manager without an external wrapper. Reverse-proxy trust remains explicit through approved proxy IP/CIDR configuration.

The dedicated Monitor state database separates schema deployment from runtime permissions. Runtime receives only schema-version read access plus SELECT/INSERT/UPDATE on the shared document table. Monitored SQL permissions are derived from the bounded collector read surface rather than broad database roles; the deployment script grants server visibility/performance-state permissions and narrow read access to the required master/msdb metadata only, with no DML/DDL/sysadmin path.

Release artifacts are created only after Release build with warnings-as-errors and the full test suite. Versioned Windows x64 packaging produces a SHA-256 checksum and uses read-only repository permissions. Deployment admission uses HTTPS `/health/live`, `/health/ready` and `/health` probes only; smoke testing never substitutes a monitored-SQL query. Upgrades retain a previous executable rollback point and an operational backup, while rollback explicitly avoids deleting key rings, secrets or shared state to force startup.
