...
| Packaged Windows Server/IIS bootstrap installer | P0.5 | In review | Issue #276 / PR #278; PLAN ONLY by default; detects/installs PowerShell 7, IIS scripting features and .NET 8 Hosting Bundle online or from offline paths; validates/imports only the approved HTTPS certificate, creates/validates low-privilege No Managed Code app pool + exact site/binding/ACLs, packages setup under `_operations`, then delegates to existing preflight/versioned deploy/rollback; repository-only and cannot satisfy #162/#116 |
