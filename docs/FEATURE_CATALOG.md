...
| First Production SingleNode release | P0.5 | Repository verified / external pending | Issue #116; IIS deployment + exact 15-gate evidence/finalization tooling verified; selected RC.43 on #116; actual trusted-HTTPS IIS/recycle/least-privilege/backup/rollback evidence still required |
...
| Immutable production acceptance session | P0.5 | In progress | Issue #150 / PR #151; fresh candidate-bound workspace verifies exact ZIP/checksum, rejects unsafe/traversal paths and secret-like metadata, creates SHA-locked manifest + canonical 15-gate pack at 0/15, and never deploys IIS/executes SQL/marks PASS/finalizes acceptance |
