...
This is the **current live feature catalog**. Exact GitHub state, `AGENTS.md`, and `docs/CURRENT_EXECUTION_PLAN.md` remain authoritative for work selection. The detailed catalog that preceded the owner-closure reconciliation is preserved byte-for-byte at `docs/history/FEATURE_CATALOG_PRE_OWNER_CLOSURE_RECONCILIATION_2026-09-09.md`.
...
| Visible portal/UI completion | BATCH-700 | COMPLETE 50/50 | Final PR #240 squash-merged as `fd33e79c6d19d7f9852417b9c35a11f91f21714c`; exact final head `0834db6b5d518fe5c52eec9b47c03e467929aa89` passed selected CI/Real-SQL/Windows gates. Responsive/accessibility source contracts remain authoritative |
...
| Shared-state atomic execution guard | #423 / PR #424 | COMPLETE / MERGED | Production SharedState read/CAS revalidates schema readiness inside the same serializable transaction; final head passed all selected gates |
...
| Optional Website Monitoring browser verification | PR #472 | CLOSED UNMERGED BY OWNER DIRECTION | The recovered branch reached Green exact-head browser/CI evidence, including run `34384824489` and artifact `10117386057`, but the owner explicitly directed close without merge. No #472 browser workflow/script/doc/test is part of `main`; it is not required for Website Monitoring or P0 closure and must not be duplicated/revived without new authoritative scope |
