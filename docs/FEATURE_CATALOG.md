...
| Fresh-host IIS + pinned PowerShell 7 prerequisite hardening | P0.5 | In validation / PR #283 | Issue #281; ports only unique fresh-host hardening from superseded PR #280 onto current main: pinned official PowerShell 7.4.16 x64 MSI Online/offline bootstrap with SHA-256 + Microsoft Corporation Authenticode, mandatory pwsh relaunch before IIS mutation, reboot fail-closed behavior, robust .NET/ANCM detection, constrained Microsoft Hosting Bundle sources, shared-IIS restart gating, PFX reuse, binding drift and conditional ACL hardening. Dedicated tests and fresh-host operator/work docs included; no RC.61/tag/release or external acceptance mutation |
