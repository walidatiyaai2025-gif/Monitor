...
| Locked-session gate/finalization binding | P0.5 | Repository hardening | Issue #258 / PR #259; externally preserved session-manifest SHA-256 is authenticated against the lock, candidate ZIP/checksum and pack candidate/environment identity; recorder/finalizer require it; validator can independently enforce it; Windows runtime uses Session → Recorder → Finalizer → Validator with pack/manifest/candidate drift negatives; exact final-head CI evidence is tracked on #258 |
