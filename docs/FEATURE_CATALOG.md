...
| Idempotent IIS bootstrap + SingleNode install entrypoint | P0.5 | In validation / PR #279 | Issue #276; repository implementation on main via ce498e1b/94e44caf/e7621fcb adds PlanOnly-by-default IIS/.NET 8 Hosting Bundle/bootstrap, Offline + constrained Microsoft Online installer modes with optional SHA-256 pin and Authenticode verification, low-privilege No Managed Code app pool/site/HTTPS/certificate/PFX/filesystem ACL handling, and one bootstrap -> authoritative preflight -> existing deploy entrypoint. PR #279 adds production-candidate parser/package integration, operator runbook coverage and canonical tracking. Existing immutable release/external App_Data/acceptance/rollback semantics remain authoritative; no RC.61 publication or real #116 mutation occurs |
