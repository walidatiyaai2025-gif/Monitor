...
| Configurable Dashboard live database status | #476 / PR #477 | COMPLETE / MERGED | Integrated as squash merge `f3dc1b2234b2105d1331d2476f75e8b7b0f2761c` from exact final head `ac9499fe54000de8a8a38865bd47d062b5985d4f` after CI `34392461369`, Real SQL `34392461363`, Windows production-candidate `34392461390`, protected-P0 commits `34392461395` and metadata `34392461372` all passed; exact merged-main CI `34392925320` passed and #476 closed completed. The panel uses cached Dashboard evidence only, bounded persisted 1/2/5/10/15/30-minute refresh, fail-closed HTML/redirect/evidence handling, no overlapping/hidden-tab polling, reduced-motion support, and no browser monitored-SQL/collector/`/refresh-snapshot` path |
| DBA Command Center + mobile DBA dashboard | `feature/dba-command-center` | IMPLEMENTED ON BRANCH / VALIDATION PENDING | Visible `/dba` surface adds explicit audited deep inspection for top query CPU/logical I/O, memory grants, per-database size/buffer-pool/recovery/backup state and missing-index evidence; recommendations expose evidence + review-only fix/investigation + validation SQL; one/all-database backup plans are generated without execution; authenticated `/api/dba/summary` feeds `apps/monitor_dba_flutter` with instance usage and what/when/where DBA actions. Normal GET/mobile paths remain cache/control-plane only; SQL credentials remain server-side |
| First-install wizard + in-app upgrades | `feature/dba-command-center` | IMPLEMENTED ON BRANCH / VALIDATION PENDING | Windows wizard installs existing Monitor Windows Service hosting, derives production PBKDF2 administrator credentials, configures port, upgrade agent and readiness validation. Admin > Monitor Upgrades verifies/stages ZIP + SHA-256; separate host updater re-verifies, backs up, swaps binaries, health-checks and attempts rollback. Running web process never replaces itself |
...
- credentials, full connection strings, secret material, raw provider errors, arbitrary SQL text, client/table data and physical paths remain excluded according to existing contracts; sanitized bounded query snippets are permitted only in the explicit DBA inspection cache and must redact literals before retention;
