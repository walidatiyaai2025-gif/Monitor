...
M3-005 adds `IHealthRecommendationService`, a pure deterministic mapping from the current allowlisted incident rule IDs to immutable `HealthRecommendation` objects. Recommendations contain a problem explanation, the bounded incident evidence, fixed rationale/confidence, ordered remediation steps and, where useful, an application-owned read-only diagnostic SQL proposal. Evidence is never interpolated into SQL. Unsupported rules return no recommendation. The recommendation route is authorized and read-only; it can reconcile from the existing cache/incident pipeline but exposes no execution endpoint. Diagnostic SQL is displayed for manual DBA review only and cannot alter monitored servers, incident state or collection frequency.
