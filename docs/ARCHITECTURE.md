...
M5-026 keeps the existing `IAuditStore` as the only operator-audit sink and enriches incident transitions at the workflow/controller boundary. `IncidentTransitionResult` carries bounded applied/rejected state context around the repository compare-and-set. The controller requires a usable authenticated principal name before invoking the workflow, so a missing identity cannot mutate incident state or produce a fabricated `unknown` actor. Successful and rejected transition outcomes are bounded state labels only; incident evidence and request payloads never enter the audit record.
